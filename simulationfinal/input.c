...
void backtrace(GATE *Node,int *gat,int *value)
{
	printf("I am here");
    LIST* tempnode;
   // i=graph[id].type;
   int num_inversion,t,x;
	 num_inversion=0;
printf("%d\n",Node[*gat].Nfi);
if (Node[*gat].Type==NOR|Node[*gat].Type==NAND|Node[*gat].Type==NOT)
        num_inversion++;
    while(Node[*gat].Nfi!=0)
    {
	tempnode=Node[*gat].Fin;
	while(tempnode!=NULL)
	{
		//printf("I am also here");
        //for(t=0;t<Node[*gat].Nfi;t++)
        //{
